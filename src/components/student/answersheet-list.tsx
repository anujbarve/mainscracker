...
import {
  ...
  IconChevronRight,
  IconEye,
  IconUserCheck, // ✅ 2. Import new icons
  IconEdit,
...
const StatusBadge = ({ status }: { status: AnswerStatus }) => {
  ...
  > = {
    ...
    // ✅ 3. ADDED "revision_requested" STATUS
    revision_requested: {
...
const columns: ColumnDef<AnswerWithDetails>[] = [
  {
    ...
    header: "Subject",
    cell: ({ row }) => (
      <div className="font-medium">{row.original.subjects?.name || "N/A"}</div>
    ),
  },
  {
  ...
  },
  {
    ...
    header: "Evaluator",
    cell: ({ row }) =>
      row.original.assigned_faculty?.full_name || (
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-right font-semibold">
        {row.original.marks_awarded !== null
          ? row.original.marks_awarded
          : "N/A"}
      </div>
    ),
  },
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              // ✅ 4. Stop click from bubbling to the row
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem asChild>
              <Link
                href={row.original.answer_file_url}
                target="_blank"
                rel="noopener noreferrer"
              >
                <IconFileDownload className="mr-2 h-4 w-4" /> Download
                Submission
              </Link>
            </DropdownMenuItem>
            {row.original.evaluated_file_url && (
              <DropdownMenuItem asChild>
                <Link
                  href={row.original.evaluated_file_url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <IconFileDownload className="mr-2 h-4 w-4" /> Download
    ...
    <main className="w-full min-h-[calc(100vh-theme(spacing.16))] p-4 sm:p-6 lg:p-8">
      <Card className="shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            A history of all your submitted answer sheets and their evaluation
            status.
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {loading ? (
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      ...
                      // ✅ 6. ADD ONCLICK AND STYLING FOR CLICKABLE ROWS
                      className="cursor-pointer hover:bg-muted/50"
                      onClick={() =>
                        router.push(`/student/answers-list/${row.original.id}`)
                      }
                    >
